...
public class FastByteOperations {
    ...
    public static final class UnsafeOperations implements ByteOperations
    {
                                    ...
                                    Object buffer2, long memoryOffset2, int length2)
        {
            ...
            for (int i = 0; i < wordComparisons ; i += 8)
            {
                ...
                {
                    if (BIG_ENDIAN)
                        return ULcompare(lw, rw);

                    return ULcompare(Long.reverseBytes(lw), Long.reverseBytes(rw));
                }
            }
    ...
    // Mimic com.google.common.primitives
    private static long flip(long a) {
        return a ^ Long.MIN_VALUE;
    }
    public static int ULcompare(long a, long b) {
        return Lcompare(flip(a), flip(b));
    }
    public static int Lcompare(long a, long b) {
